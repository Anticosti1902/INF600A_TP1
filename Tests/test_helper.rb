...
class Object
  ...
  def it_( test, niveau = :base, &bloc )
    if niveau_a_tester = ENV['NIVEAU']  #!!
      return unless [niveau, :tous].include?( niveau_a_tester.to_sym )
    end
...
#
# Execute le script ./gv.sh avec commande, options et arguments
...
#
# @param [String] cmd La commande a executer avec ses options et arguments  (sans './gv.sh')
# @return [Array[Array<String>, Array<String>, Fixnum] Les lignes produites sur stdout, stderr et le code de statut
#
def run_gv( *cmds )
  cmd_line = cmds.map{ |cmd| "./gv.sh #{cmd}" }.join( ' | ' )
